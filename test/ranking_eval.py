...
def eval_adaptive_mae(dataset, map_path):
    print("Calculating Adaptive MAE...")
    ...
    for i in range(num):
        ...
        mae = Eval_adaptive_mae(gt_mask, pred_mask)

        mae_list.append(mae)

    print("\n")
    avg_mae = sum(mae_list) / len(mae_list)
    print("Average MAE Images = ", avg_mae.item())
    return avg_mae
...
def calculate_spr(dataset, model_pred_data_path):
    ...
    for i in range(num):
        ...
        img = Image.open(os.path.join(model_pred_data_path, image_id + ".png"))
...
def eval_sor(map_path, dataset_root='datasets/ASSR/'):
    data_split = "test"
    dataset = DatasetTest(dataset_root, data_split, eval_spr=True)
