...
class Eval_thread():
    ...
    def run(self):
        ...
        adaptive_mae, adaptive_mae_fg, adaptive_mae_bg = self.Eval_adaptive_mae()
        Res['adaptiveMAE'] = adaptive_mae
        print(adaptive_mae)
        ...
        self.LOG(
            ...
            .format(self.dataset, self.method, mae, max_f, mean_f, max_e, mean_e, s))

        return '[cost:{:.4f}s]: {:.4f} adaptive_mae || {:.4f} mae || {:.4f} max-fm || {:.4f} max-Emeasure || {:.4f} S-measure.'.format(
            time.time() - start_time, adaptive_mae, mae, max_f, max_e, s)
    ...
    def Eval_adaptive_mae(self):
        print('eval[adaptive MAE]:{} dataset with {} method.'.format(self.dataset, self.method))
        avg_mae, avg_mae_fg, avg_mae_bg = 0.0, 0.0, 0.0
        img_num, img_num_fg, img_num_bg = 0.0, 0.0, 0.0
        ...
        with torch.no_grad():
            ...
            for pred, gt in self.loader:
                ...
                if torch.any(mask_fg):
                    mea_fg = torch.abs(pred[mask_fg] - gt[mask_fg]).mean()
                    if mea_fg == mea_fg:  # for Nan
                        avg_mae_fg += mea_fg
                        img_num_fg += 1.0

                # background:
                mask_bg = (gt == 0)
                mae_bg = torch.abs(pred[mask_bg] - gt[mask_bg]).mean()
                if mae_bg == mae_bg:  # for Nan
                    avg_mae_bg += mae_bg
                    img_num_bg += 1.0

            avg_mae_fg /= img_num_fg
            # print("img_num_fg: {}".format(img_num_fg))
            avg_mae_bg /= img_num_bg
            # print("img_num_bg: {}".format(img_num_bg))

            avg_mae = 0.5 * avg_mae_fg + 0.5 * avg_mae_bg

            return avg_mae.item(), avg_mae_fg.item(), avg_mae_bg.item()
